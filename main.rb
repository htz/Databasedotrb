...
require 'optparse'
require './databasedotrb.rb'

@options = {}
OptionParser.new do |opt|
	Version = '0.0.1'
	opt.on('-u', '--username USERNAME', 'Login username.') {|v| @options[:username] = v}
	opt.on('-p', '--password  PASSWORD', 'Login password.') {|v| @options[:password] = v}
...
Databasedotrb::CLI.new 'databasedotcom.yml', @options do |dbdc|
	dbdc.login.start
end
